...
module.exports.run = async (client, message, args) => {
    ...
    if (args.length == 0) {
        client.channels.get("477189452544999435").send("A player needs a name. `!rank <YOUR BATTLE TAG>`")
        return;
    }
    while (args[0] == '') {
        args = args.slice(1);
    ...
    OW.getProfile(platform, region, user, (err, json) => {
        ...
        if (err instanceof TypeError) {
           ...
           client.channels.get("477189452544999435").send(`I couldn't find ` + user + ` because of a [` + err.name + `] - ` + err.message + " :frowning:")
            ...
            client.channels.get("477189452544999435").send(user + "'s competitive rank is " + json.competitive.rank);
        }
    });
