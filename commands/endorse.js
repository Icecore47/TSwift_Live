...
module.exports.run = async (client, message, args) => {
    if (args.length == 0){
        client.channels.get("477189452544999435").send("A player needs a name. `!endorse <YOUR BATTLE TAG>`")
        return;
    }
    while (args[0] == '') {
        args = args.slice(1);
    ...
    OW.getProfile(platform, region, user, (err, json) => {
        ...
        if (err instanceof TypeError) {
           ...
           client.channels.get("477189452544999435").send(user + "'s endorsement level is " + json.endorsement.level + ":\n" +
